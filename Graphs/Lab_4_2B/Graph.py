...
class Graph(object):
    ...
    def modified_BFS(self, start_node, dest_node):
        """
        Returns the number of distinct walks between 2 given vertices
        """
        res = self.toposort()
        if res == None:
            return None
        ...
        while queue:
            ...
            if node == dest_node:
                nr_distinct_walks += 1
            node_neighbours = self.neighboursOfXOut(node)
            for neighbour in node_neighbours:
                queue.append(neighbour)
        
        return nr_distinct_walks
    
    def distinct_walks(self, start_node, dest_node):
        """
        Returns the number of distinct walks between 2 given vertices
        """
        tsorted = self.toposort()

        d = []      #d[x] = y means there are y distinct walks up to that node  
